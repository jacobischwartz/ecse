...
<div class="style-emails wrap">
	...
	<h2><?php _e("WP e-Commerce Style Emails", 'ecse'); ?></h2>
	<br />
	<div style="width:420px; border-bottom:1px solid #CCCCCC;">
		<div class='list-item create-instructions'>
			1. <b> <?php _e('Create', 'ecse'); ?> </b> <?php _e('the', 'ecse'); ?> <i>wpsc-email_style.php</i> <?php _e('wrapper template file in your theme', 'ecse'); ?>:<br />
			<?php 
			...
			?>/<br />
			<br />
			<?php _e('Or copy a sample', 'ecse'); ?> <i>wpsc-email_style.php</i> <?php _e('from this plugin\'s "theme" subfolder', 'ecse') ?>:<br />
			<?php 
			...
			?><br />
			<br />
			<?php echo __("Click ", 'ecse')."<a id='ecse_theme_editor_link' href='#'>".__("here", 'ecse')."</a> ".__("to use the in-page coding editor", 'ecse'); ?>.
			<div id="expandable_tips_show" class="expandable_tips_control"><?php _e("Show docs", 'ecse'); ?><img src="<?php echo plugins_url('images/down_arrow_blue.gif',__FILE__) ?>" /></div>
			<div id="expandable_tips_hide" class="expandable_tips_control"><?php _e("Hide docs", 'ecse'); ?><img src="<?php echo plugins_url('images/up_arrow_blue.gif',__FILE__) ?>" /></div><br /><br />
			<div id="expandable_tips">
				<?php _e("This is a simple example of what you could put in your template file", 'ecse'); ?>:<br />
...
&lt;/body>&lt;/html>
				...
				<?php _e('In addition, these are some', 'ecse'); ?> <a href="http://codex.wordpress.org/Conditional_Tags" target="_new"><?php _e('conditional tags', 'ecse');?></a> <?php _e('you can use', 'ecse'); ?>:<br />
				<pre>
...
ecse_is_other_email()
				...
				<?php _e('Want to get really fancy? This plugin now supports a', 'ecse') ?> <a href="http://codex.wordpress.org/Template_Hierarchy" target="_new"><?php _e('template hierarchy', 'ecse');?></a> <?php _e('for the wrapper, and content replacement for improved receipt emails!', 'ecse'); ?><br />
				...
				<br />
				<?php _e('Here are good references for designing html/css emails:', 'ecse'); ?> <a href="http://www.campaignmonitor.com/css/" target="_new">CampaignMonitor</a> | <a href="http://mailchimp.com/resources/guides/html/email-marketing-field-guide/" target="_new">MailChimp</a>
			</div>
			<?php do_action('ecse_options_page_create_instructions'); ?>
		</div>
		<div class='list-item preview-options'>
			2. <b><?php _e('Preview', 'ecse'); ?></b> <?php _e('your styling.', 'ecse'); ?><br /><br /><?php _e("Live WPEC emails sent to an admin will automatically get styled, even when live styling is turned off. So feel free to run a test purchase with your admin email address, and preview exactly what customers will see.", 'ecse'); ?><br /><br /><?php _e("You can also have a very simple styled email sent to you:", 'ecse'); ?><br />
			<?php 
				global $current_user;
				get_currentuserinfo();
			?>
			<form action="" method="post" id="email-style-testing-form" name="email-style-testing-form">
				...
				<?php _e('Or preview the email from your browser', 'ecse') ?>: &nbsp; <a href="<?php echo add_query_arg('ecse_preview','true'); ?>" id="ecse_activate_preview" target="_new"><?php _e('general email', 'ecse'); ?></a> | <a href="<?php echo add_query_arg(array('ecse_preview'=>'true','ecse_type'=>'receipt')); ?>" target="_new"><?php _e('purchase receipt', 'ecse'); ?></a>.<br />
				<?php _e('(Good for quick previews, but don\'t only rely on this. Email clients can render your html differently!)', 'ecse') ?>
				<?php if(!empty($_POST['send-email-submit'])) echo '<span class="sent-message">' . __('Sent', 'ecse') . '.</span>'; ?>
			</form>
			<?php do_action('ecse_options_page_preview_options'); ?>
		</div>
		<div class='list-item turn-on-options'>
			3. <b><?php _e('Turn on', 'ecse'); ?></b> <?php _e('the live email styling when you\'re satisfied.', 'ecse'); ?><br />
			<form action="" method="post" id="email-style-options-form" name="email-style-options-form">
				<input type="checkbox" name="style_emails_store" id="style_emails_store" <?php if(get_option('ecse_is_active')) { echo 'checked="checked"';  }; ?> /><label for="style-emails-active"><?php _e("Style live customer emails", 'ecse'); ?></label><br />
				<input type="checkbox" name="style_emails_other" id="style_emails_other" <?php if(get_option('ecse_is_other_active')) { echo 'checked="checked"';  }; ?> /><label for="style-other-emails-active"><?php _e("Style all other website emails (contact forms, user registrations, etc)", 'ecse'); ?></label><br />
				<br />
				<?php _e('Exclude emails by subject. Enter each subject on its\' own line:', 'ecse') ?><br />
				...
				<?php if(!empty($_POST['save-options-submit'])) echo '<span class="sent-message">'.__('Saved', 'ecse').'.</span>'; ?>
			</form>
	...
	<form action="" method="post" id="email-style-wish-form" name="email-style-wish-form">
		<h3><?php _e("Contact this plugin's developer", 'ecse'); ?></h3>
		<?php if(!empty($_POST['send-wish-submit'])) echo '<div class="sent-message">Sent.</div>'; ?>
		<textarea name="send_wish_content"><?php _e("Write some constructive criticism or positive feedback", 'ecse'); ?></textarea>
		...
		<?php _e("or visit", 'ecse'); ?> <a href="http://schwambell.com"><?php _e("his website", 'ecse'); ?></a> | <a href="http://wordpress.org/extend/plugins/wp-e-commerce-style-email/">WP.org <?php _e('plugin page', 'ecse') ?></a> | <a href="http://wordpress.org/tags/wp-e-commerce-style-email">WP.org <?php _e('forums', 'ecse') ?></a>
	</form>
	
	<div id="ecse_theme_editor_container" class="ecse_overlay">
		<div id="ecse_theme_editor_inner" class="inner">
			<h2><?php _e('Theme Template File', 'ecse'); ?></h2>
			<textarea id="ecse_theme_editor"></textarea><br />
			<div id="ecse_theme_editor_controls">
				<input type="submit" id="ecse_theme_editor_submit" value="<?php _e('save', 'ecse'); ?>" class="submit_button"><div id="ecse_theme_editor_loading" class="esce_loading"></div> 
				<a href="#" id='ecse_theme_editor_revert'><?php _e('Revert', 'ecse'); ?></a> | 
				<a href="#" id='ecse_theme_editor_cancel'><?php _e('Cancel', 'ecse'); ?></a>
			</div>
		</div>
	</div>
	...
	jQuery(document).ready(function(){
		...
		jQuery('#ecse_theme_editor_link').click(function() {
			...
			jQuery('#ecse_theme_editor').val('<?php _e('loading', 'ecse'); ?>...');
			getThemeFile();
		...
		jQuery('#ecse_theme_editor_container').click(function(event) { //clicking outside the editor
			if( event.target == this ) {
				if( hasChanges('<?php _e("Are you sure you want to close the editor? You will lose any unsaved changes", 'ecse'); ?>') ) jQuery(this).fadeOut('slow');
			}
		});
		jQuery('#ecse_theme_editor_cancel').click(function() { //clicking the cancel link
			if( hasChanges('<?php _e("Are you sure you want to close the editor? You will lose any unsaved changes", 'ecse'); ?>') ) jQuery('#ecse_theme_editor_container').fadeOut('slow');
		});

		//editor's revert link
		jQuery('#ecse_theme_editor_revert').click(function() {
			if( hasChanges('<?php _e("Are you sure you want to revert? You will lose any unsaved changes", 'ecse'); ?>') ) {
				jQuery('#ecse_theme_editor').val('<?php _e('loading', 'ecse'); ?>...');
				getThemeFile();
			}
		});
	...
	function saveThemeFile() {
		...
		jQuery.post(ajaxurl, data, function(response) {
			if(response=='false') {
				display_notification('<?php _e("There was a problem saving your theme file", 'ecse');?>');
			} else {
	...
	function getThemeFile() {
		...
		jQuery.post(ajaxurl, data, function(response) {
			if(response=='false') { //theme file does not yet exist
				...
				display_notification('<?php _e("Click SAVE to create your theme file", 'ecse');?>');
			}
	...
	function SendFeedback(feedback) {
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if(response=='false') notification='<?php _e("There was a problem sending that message", 'ecse');?>';
	...
	function SaveOptions(option_values) {
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if(response=='false') notification='<?php _e("There was a problem saving the settings", 'ecse');?>';
			display_notification(notification);
	...
	function SendPreview(email_address) {
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if(response=='false') notification='<?php _e("There was a problem sending that preview", 'ecse');?>';
			display_notification(notification);
